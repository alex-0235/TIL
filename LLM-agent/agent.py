...
def build_agent_executor():
    ...
    loader = PyMuPDFLoader('./ESGReport2024_KR.pdf')
    docs = loader.load()
    ...
    system_text = f"""
    너는 웹 검색이 가능하고, 2024년 카카오 ESG 리포트 정보를 담은 pdf 를 검색할 수 있는 어시스턴트야.
    - 사용자가 PDF 문서와 관련된 질문(ex. '카카오', '이 pdf에서', '문서내용', '파일에서)을 하면 반드시 'pdf_search' 도구를 써야해
    - 사용자 질문이 팩트체크를 필요로 하고, 최신성이 필요하다 판단되면 web_search를 실행해야해
    - 사용자가 일반적인 질문을 하고, 최신성이나 팩트체크가 필요없으면 그냥 답변해
    - 뭔가 확실하지 않으면 web_search을 먼저 실행하고 그 다음 pdf_search를 실행해서 답변을 생성해
    오늘은 {today} 야.
    """
